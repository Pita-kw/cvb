...
function putAttachedElementsOnBones()
	for element,ped in pairs(attached_ped) do
		...
		elseif isElementStreamedIn(ped) then
			...
			if not isNaN(objx) and not isNaN(objy) and not isNaN(objz) and not isNaN(offrx) and not isNaN(offry) and not isNaN(offrz) then 
				setElementPosition(element,objx,objy,objz)
				setElementRotation(element,offrx,offry,offrz,"ZXY")
				setElementDimension(element, getElementDimension(ped))
				setElementInterior(element, getElementInterior(ped))
			end
		else
