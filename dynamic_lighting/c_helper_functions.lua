...
-- light sorting
---------------------------------------------------------------------------------------------------
function sortedOutput(inTable,isSo,distFade,maxEntities,camDim,camInt)
	local outTable = {}
	for index,value in ipairs(inTable) do
		if inTable[index].enabled and ((value.dimension < 0 and value.interior < 0) or (value.dimension == camDim and value.interior == camInt)) then
			...
			if dist <= distFade then 
				...
				outTable[w].interior = value.interior 
				outTable[w].dimension = value.dimension
...
---------------------------------------------------------------------------------------------------
function createLightShaderFiles(currLightNr, currVertLightNr, isLayer, isWrdSobel, isNightMod, isPedDiffuse, isShading, isDirLight, isDiffuse, isNightSpot,isSM3,isForceVertex)
	local isValid = true
	...
	if isValid then
		...
		outputDebugString('Vertex lights forced WRD: '..tostring(isForceVertex[1])..' VEH: '..tostring(isForceVertex[2])..' PED:'..tostring(isForceVertex[3]))
		outputDebugString('Effect layered WRD: '..tostring(isLayer[1])..' VEH: '..tostring(isLayer[2])..' PED: '..tostring(isLayer[3]))
		outputDebugString('Effect normal WRD: '..tostring(isShading[1])..' VEH: '..tostring(isShading[2])..' PED: '..tostring(isShading[3]))
...
function createEffectFile(filePath, lightNr, verLightNr, isLayer, isSobel, isNightMod, isGTADiffuse, isShading, isDirLight, isDiffuse, isVehicle, isTex1, isNightSpot, isVertexForced)
	...
	if (theFile) then
		...
		--Lighting
		if isShading or isVertexForced or (verLightNr > 0) then
		...
		if isVertexForced or (verLightNr > 0) then
			writeFileLine( theFile, ' float4 Lighting : TEXCOORD6; ')		
		end
		...
		if isShading or (verLightNr > 0) then
			writeFileLine( theFile, ' PS.Normal = mul(VS.Normal, gWorldInverseTranspose); ')
			if isDirLight then
				writeFileLine( theFile, ' PS.VertLight = createDirLight( normalize(PS.Normal), gDirLightDirection, gDirLightDiffuse) * saturate(MTAUnlerp(gDirLightRange, gDirLightRange * 0.6, DistanceFromCamera)); ')
			end
			if isSobel then
		...
		if verLightNr > 0 or isVertexForced then
			writeFileLine( theFile, ' PS.Lighting = 0; ')
			local verLightStart = math.max(lightNr - verLightNr + 1, 0)
			local verLightStop = math.max(verLightStart + verLightNr - 1, 0)
			verLightStop = math.max(verLightStop, verLightStart)
			if isVertexForced then
				verLightStart = 0
				verLightStop = lightNr
			end
			for lID=verLightStart,verLightStop do
				writeFileLine( theFile, ' if (gLight'..lID..'Enable) PS.Lighting += createLight(PS.Normal, PS.WorldPos, gLight'..lID..'Type, gLight'..lID..'Position, gLight'..lID..'Direction, gLight'..lID..'Diffuse, gLight'..lID..'Attenuation, gLight'..lID..'Phi, gLight'..lID..'Theta, gLight'..lID..'Falloff, gLight'..lID..'NormalShadow ); ')		
			end
			writeFileLine( theFile, ' PS.Lighting = saturate(PS.Lighting); ')
		end		
		...
		if isGTADiffuse and isShading and not isLayer then
			if isVehicle then
				writeFileLine( theFile, ' float4 Diffuse = MTACalcGTAVehicleDiffuse( PS.Normal , VS.Diffuse );')
			else
				writeFileLine( theFile, ' float4 Diffuse = MTACalcGTAPedDiffuse( PS.Normal , VS.Diffuse );')			
			end
			if isVehicle then
				if isDiffuse or isDirLight then
					writeFileLine( theFile, ' PS.VertLight += sMaterialSpecular * MTACalculateSpecular( sCameraDirection, sLight1Direction, PS.Normal, sMaterialSpecPower ) * 0.7 * max( gDayTime, PS.VertLight.a ); ')
				else
					writeFileLine( theFile, ' PS.VertLight = sMaterialSpecular * MTACalculateSpecular( sCameraDirection, sLight1Direction, PS.Normal, sMaterialSpecPower ) * 0.7 * gDayTime * max( gDayTime, PS.VertLight.a ); ')				
				end
			end	
		else
		...
		if (lightNr - verLightNr) > -1 and not isVertexForced then
			local genLight = math.max(lightNr - verLightNr, 0)
			for lID=0,genLight do
		...
		if (verLightNr > 0) or isVertexForced then
			writeFileLine( theFile, ' texLight += PS.Lighting; ')
		end
		if isLayer then
