...
local function doPoolShot(x,y,x2,y2)
	...
	if not hit or not hitElement then
		return triggerServerEvent("broadcastCaptionedEvent", localPlayer, getPlayerName(localPlayer) .. " nie trafił w nic.", 5, 5, true)
	end
	local model=getElementModel(hitElement)
	if (ballNames[model]) then
		triggerServerEvent("broadcastCaptionedEvent", localPlayer, getPlayerName(localPlayer) .. " trafia w "..ballNames[model]..".", 5, 5, true)
	end
