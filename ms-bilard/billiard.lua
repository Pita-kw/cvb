...
addEventHandler("onColShapeHit", resourceRoot, function(el,md)
	...
	if st and st=="pot" then
		...
		if ballNames[model] and deleteBall(el, idx) then
			triggerEvent("broadcastCaptionedEvent", source, ballNames[model].." wpadła do dziury.", 5, 5, true)
		end
		return
	end
...
addCommandHandler("resettable",function(plr,cmd,tn)
	...
	if not tn then
		outputChatBox("Użyj: /resettable <numer stołu>", plr)
		return
	end
...
--[[
	...
	createBalls(tn)
	triggerEvent("broadcastCaptionedEvent", plr, getPlayerName(plr) .. " zresetował kule na bilardzie.", 5, 5, true)
end)                                                                   