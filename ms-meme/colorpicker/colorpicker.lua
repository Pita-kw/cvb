...
colorPicker = {
	...
	constructor = function( info )
		...
		colorPicker.GUI.selectWindow = guiCreateWindow(screenW - info.selectWindow.width, (screenH - info.selectWindow.height) / 2,
		                                        info.selectWindow.width, info.selectWindow.height, "Pick a color (click outside window to close)", false)
...
--Wont work until I get the actual mouse working
		...
		for i=1,3 do
		  for j=1,3 do
		  	...
		  	local x = wx + ((boxWidth + 5) * (j-1))
			local y = wy + 30 + ((boxHeight + 5) * (i-1))
	...
	pickColor = function( button, state, cursorX, cursorY )
		...
		minX, minY, maxX, maxY = wx,
		                         wy,
					 wx + colorPicker.selectWindow.historyWidth,
					 wy + colorPicker.selectWindow.historyHeight
