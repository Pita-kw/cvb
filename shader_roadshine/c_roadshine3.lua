...
function switchRoadshine3( bOn )
	if bOn then
		enableRoadshine3()
		if getElementData(localPlayer, "player:shader_detail") then 
			triggerEvent("onClientAddNotification", localPlayer, "Ten shader nie jest kompatybilny z detalami tekstur.", "warning")
		end
	else
		disableRoadshine3()
	end
end
addEvent( "switchRoadshine3", true )
addEventHandler( "switchRoadshine3", resourceRoot, switchRoadshine3 )
