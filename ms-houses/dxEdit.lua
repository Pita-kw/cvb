...
function createEditBox( id, xS, yS, w, h, widoczny, tekst, zamaskowany, dlugosc, czcionka, spacja, typ, kolor, prostokat, kolorProstokat, skala, clip, ms, tekstBool, tekstPomocniczy, tekstPomocniczyKolor, maZniknac, skalaPomoc, czcionkaPomoc, postGUI, wstawka, kolorWstawki, specjalneZnaki, mozeKliknac)
	if dxEditBox[id] == nil then
		...
		dxEditBox[id].skalaPomoc = (skalaPomoc/x) * x
...
function changeEditBoxHelpTextAlpha (id, tabelka)
	if dxEditBox[id] ~= nil then
		dxEditBox[id].tekstPomocniczyKolor = {dxEditBox[id].tekstPomocniczyKolor[1], dxEditBox[id].tekstPomocniczyKolor[2], dxEditBox[id].tekstPomocniczyKolor[3], tabelka}
	end
end
			...
			if dxEditBox[k].aktywny then
				...
				sekundy = getTickCount() / 400
					if dxEditBox[k].dlugoscDX < ( x * ( dxEditBox[k].x +dxEditBox[k].w-0.004 )) then	
						...
						dxDrawRectangle(math.floor(x * (dxEditBox[k].x + dxEditBox[k].w/2)), y * (dxEditBox[k].y+0.01), x * ( 0.001 ), y * ( dxEditBox[k].h * 0.6 ), tocolor( dxEditBox[k].kolorWstawki[1], dxEditBox[k].kolorWstawki[2], dxEditBox[k].kolorWstawki[3], math.abs(math.sin(sekundy) * 220)), dxEditBox[k].postGUI)			
						else
						dxDrawRectangle(math.floor(dxEditBox[k].dlugoscDX), y * (dxEditBox[k].y+0.01), x * ( 0.001 ), y * ( dxEditBox[k].h * 0.6 ), tocolor( dxEditBox[k].kolorWstawki[1], dxEditBox[k].kolorWstawki[2], dxEditBox[k].kolorWstawki[3], math.abs(math.sin(sekundy) * 220)), dxEditBox[k].postGUI)			
						end
					end				
				end
			end
...
function zwrocWartoscPrzesuniecia ( k, bool ) 
	if bool ~= true then
	local maxCarret = math.floor(x * (dxGetTextWidth( dxEditBox[k].tekst, dxEditBox[k].skala, dxEditBox[k].czcionka )/x))
	local pozycjaTekstuDX = math.floor ( x * ( (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/x )+0.001  ))
	dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
	dxEditBox[k].maxTextDX = maxCarret
	return  math.abs(math.floor(x * ((dxGetTextWidth( dxEditBox[k].tekst, dxEditBox[k].skala, dxEditBox[k].czcionka )/x)+pozycjaTekstuDX)))
		else
		local maxCarret = math.floor(x * (dxGetTextWidth( (dxEditBox[k].tekst:gsub(".","*")), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))
		local pozycjaTekstuDX = math.floor ( x * ( (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/x )+0.001  ))
		dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
		dxEditBox[k].maxTextDX = maxCarret
		return  math.abs(math.floor(x * ((dxGetTextWidth( dxEditBox[k].tekst:gsub(".","*"), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))+pozycjaTekstuDX))
	end
end
...
function zwrocWartoscPrzesunieciaAktywnego ( k, bool ) 
	...
	dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
	dxEditBox[k].maxTextDX = maxCarret
	return  math.abs(math.floor(x * ((dxGetTextWidth( (dxEditBox[k].tekst:sub( 1, dxEditBox[k].pozycjaCarretu)), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))+pozycjaTekstuDX))
		else
		local maxCarret = math.floor(x * (dxGetTextWidth(( dxEditBox[k].tekst:gsub(".","*")), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))
		local pozycjaTekstuDX = math.floor ( x * ( (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/x )+0.001  ))
		dxEditBox[k].dlugoscTextuDX = pozycjaTekstuDX
		dxEditBox[k].maxTextDX = maxCarret
		return  math.abs(math.floor(x * ((dxGetTextWidth( ((dxEditBox[k].tekst:sub( 1, dxEditBox[k].pozycjaCarretu)):gsub(".","*")), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))+pozycjaTekstuDX))
	end
end
			...
			if dxEditBox[k].widoczny then
				...
				if sprawdzObszar( x * dxEditBox[k].x, y * dxEditBox[k].y, x * dxEditBox[k].w, y * dxEditBox[k].h ) then
					if dxEditBox[k].aktywny == false then
						...
						ms = dxEditBox[k].ms					
						if dxEditBox[k].zamaskowany then
							maxCarret = math.floor(x * (dxGetTextWidth( (dxEditBox[k].tekst:sub(1, #dxEditBox[k].tekst)):gsub(".","*"), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))
							pozycjaTekstu = math.floor ( x * ( (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/x )  ))
						else
							maxCarret = math.floor(x * (dxGetTextWidth( dxEditBox[k].tekst:sub(1, #dxEditBox[k].tekst), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))
							pozycjaTekstu = math.floor ( x * ( (dxEditBox[k].x + dxEditBox[k].w/2 - (maxCarret/2)/x )  ))
						end
						...
						dxEditBox[k].carretPrzesuniety = false
							if dxEditBox[k].zamaskowany then
								pozycjaCarretu = math.abs(math.floor(x * ((dxGetTextWidth( (dxEditBox[k].tekst:sub(1, i)):gsub(".","*"), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))+pozycjaTekstu))
								else
									pozycjaCarretu = math.abs(math.floor(x * ((dxGetTextWidth( dxEditBox[k].tekst:sub(1, i), dxEditBox[k].skala, dxEditBox[k].czcionka )/x))+pozycjaTekstu))
								end
...
function loadDXEdit()
	addEventHandler( "onClientHUDRender", root, renderEditBoxow )
...
function unloadDXEdit()
	removeEventHandler( "onClientHUDRender", root, renderEditBoxow )
