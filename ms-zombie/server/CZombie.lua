...
function Zombie:UpdateSprint()
	if not(isElement(self.target)) then
		...
		if self.target then 
			setElementSyncer(self.ped, self.target)
		end 
...
function Zombie:Constructor(x, y, z, model, dim, int)
	...
	-- Ped halt
	self.ped = createPed(model, x, y, z, math.random(0, 360))
	...
	--	self.object = self
		...
		self.bigcol = createColSphere(x, y, z, 40)
		self.smallcol = createColSphere(x, y, z, 3)
	...
	--	setElementData(self.ped, "object", self)
		...
		addEventHandler("onColShapeHit", self.bigcol, function(target) self:SprintToPlayer(target) end) -- 
...
function Zombie:Destructor(killer)
	...
	setTimer(function() if(isElement(self.ped)) then destroyElement(self.ped) self = nil end end, 1*60*1000, 1)
	
	triggerClientEvent(getRootElement(), "doZombieSound", getRootElement(), self.ped, "dead"..math.random(1, 2));
	exports["ms-stats"]:addPlayerStat(killer, "player:zombie_kills", 1)		
end
