...
class "CPianoManager" 
{
	__init__ = function(self)
		...
		for k,v in ipairs(PIANOS) do 
			...
			if SHOW_BLIPS then 
				local piano_blip = createBlip(v[1], v[2], v[3], BLIP_ID, 2, 255, 255, 255, 255, 0, 200)
				setElementData(piano_blip, "blipIcon", "music")
			end
		end
	end,
	
	createPiano = function(self, x, y, z, rot, int, dim, r, g, b)
		if not x or not y or not z or not rot then outputChatBox("Błąd ładowania") return false end 
		...
		addEventHandler("onVehicleStartEnter", vehicle, function(enteringPlayer, seat, jacked) if jacked then cancelEvent() end end)
